...
def build_svg(results: list[ScenarioResult]) -> str:
  ...
  <text x="{left}" y="{y + 52}" class="detail">raw {result.raw_context_tokens:,} -> prompt {result.handoff_prompt_tokens:,} tokens · chunks {result.retrieved_chunk_count} · risks {result.risk_count}</text>
...
"""
  ...
  <title id="title">Context Capsule Performance Comparison</title>
  <desc id="desc">Token reduction and auto-start gate results for MVP validation scenarios.</desc>
  <style>
    .bg {{ fill: #f8fafc; }}
  ...
  <text x="32" y="42" class="title">Context Capsule Performance Comparison</text>
  <text x="32" y="68" class="subtitle">MVP scenario validation: token reduction, retrieved chunks, risk count, and auto-start gate.</text>
  {''.join(rows)}
</svg>
"""
