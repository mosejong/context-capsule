...
Context Capsule v0.1.3 is distributed as a GitHub Release ZIP.

The ZIP is a source-style local app package. It contains the Streamlit dashboard, CLI, launcher scripts, docs, tests, and release notes. It does not contain `.venv`, generated `outputs`, cache folders, build artifacts, or local credentials.
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File scripts\build_release.ps1 -Version 0.1.3 -DryRun
```
...
```text
Download context-capsule-v0.1.3.zip
Extract
...
- [ ] `scripts/run_dashboard.ps1 -DryRun` prints the Streamlit command.
- [ ] `scripts/build_release.ps1 -Version 0.1.3` creates the ZIP.
...
- v0.1.3 is not a single executable.
- Python 3.13 is required on the user's machine.
