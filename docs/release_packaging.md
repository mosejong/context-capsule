...
Context Capsule v0.2.2 is distributed as a GitHub Release ZIP.

The ZIP is a source-style local app package. It contains the FastAPI Korean local UI, CLI, launcher scripts, docs, tests, release notes, and Beta Feedback Loop documentation. It does not contain `.venv`, generated `outputs`, cache folders, build artifacts, or local credentials.
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File scripts\build_release.ps1 -Version 0.2.2 -DryRun
```
...
```text
Download context-capsule-v0.2.2.zip
Extract
...
- [ ] `scripts/build_release.ps1 -Version 0.2.2` creates the ZIP.
...
- v0.2.2 is not a single executable.
