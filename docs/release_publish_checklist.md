...
Use this file when publishing `Context Capsule v0.1.5` on GitHub Releases.
...
Tag: v0.1.5
Title: Context Capsule v0.1.5
Asset: dist/context-capsule-v0.1.5.zip
Release body: docs/releases/v0.1.5.md
```

## Before Upload

- [ ] Confirm `git status -sb` is clean.
- [ ] Confirm tag `v0.1.5` exists on GitHub.
- [ ] Confirm `dist/context-capsule-v0.1.5.zip` exists locally.
- [ ] Confirm ZIP was built with `scripts/build_release.ps1 -Version 0.1.5`.
- [ ] Confirm ZIP excludes `.venv`, `outputs`, `dist`, caches, and credentials.
- [ ] Copy the body from `docs/releases/v0.1.5.md`.
...
3. Select tag `v0.1.5`.
4. Set release title to `Context Capsule v0.1.5`.
5. Paste `docs/releases/v0.1.5.md` into the release description.
6. Upload `dist/context-capsule-v0.1.5.zip`.
7. Check that the asset name is exactly `context-capsule-v0.1.5.zip`.
8. Publish release.
...
- [ ] Confirm `README.md`, `docs/releases/v0.1.5.md`, and `docs/local_app.md` exist.
...
v0.1.5 ships as a Windows ZIP with a dashboard, CLI, GitHub Issue dry-run, Request Understanding, indexed retrieval, Korean-to-English domain retrieval hints, context redaction for prompt-injection/secret-like text, Python 3.11+ support, KDT beta quickstart, and validation reports.
```
