...
Use this file when publishing `Context Capsule v0.1.7` on GitHub Releases.
...
Tag: v0.1.7
Title: Context Capsule v0.1.7
Asset: dist/context-capsule-v0.1.7.zip
Release body: docs/releases/v0.1.7.md
```

## Before Upload

- [ ] Confirm `git status -sb` is clean.
- [ ] Confirm tag `v0.1.7` exists on GitHub.
- [ ] Confirm `dist/context-capsule-v0.1.7.zip` exists locally.
- [ ] Confirm ZIP was built with `scripts/build_release.ps1 -Version 0.1.7`.
- [ ] Confirm ZIP excludes `.venv`, `outputs`, `dist`, caches, and credentials.
- [ ] Copy the body from `docs/releases/v0.1.7.md`.
...
3. Select tag `v0.1.7`.
4. Set release title to `Context Capsule v0.1.7`.
5. Paste `docs/releases/v0.1.7.md` into the release description.
6. Upload `dist/context-capsule-v0.1.7.zip`.
7. Check that the asset name is exactly `context-capsule-v0.1.7.zip`.
8. Publish release.
...
- [ ] Confirm `README.md`, `docs/releases/v0.1.7.md`, and `docs/local_app.md` exist.
...
v0.1.7 ships as a Windows ZIP with a dashboard, CLI, GitHub Issue dry-run, Request Understanding, indexed retrieval, Korean-to-English domain retrieval hints, context redaction, first-tester dashboard guidance, intent-aware docs/launcher retrieval tuning, Python 3.11+ support, and validation reports.
```
