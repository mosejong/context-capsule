...
Context Capsule v0.2.15 is distributed as a GitHub Release ZIP.
...
```text
dist/context-capsule-v0.2.15.zip
```

Dry-run:

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File scripts\build_release.ps1 -Version 0.2.15 -DryRun
```
...
Release notes: docs/releases/v0.2.15.md
```
...
```text
Download context-capsule-v0.2.15.zip
Extract
...
If setup fails, v0.2.15 writes logs under `outputs\logs` and prints Korean guidance instead of leaving only a raw PowerShell exception.
...
- [ ] `scripts/build_release.ps1 -Version 0.2.15` creates the ZIP.
...
- v0.2.15 is not a single executable.
- v0.2.15 still uses a terminal-backed PowerShell launcher.
