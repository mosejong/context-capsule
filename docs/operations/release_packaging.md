...
Context Capsule v0.2.16 is distributed as a GitHub Release ZIP.
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File scripts\build_release.ps1 -Version 0.2.16
```

Output:

```text
dist/context-capsule-v0.2.16.zip
```

Dry-run:

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File scripts\build_release.ps1 -Version 0.2.16 -DryRun
```
...
Release notes: docs/releases/v0.2.16.md
```
...
```text
Download context-capsule-v0.2.16.zip
Extract
...
If setup fails, v0.2.16 writes logs under `outputs\logs` and prints Korean guidance instead of leaving only a raw PowerShell exception.
...
- [ ] Launcher/install scripts mention `outputs\logs` and Korean failure guidance.
- [ ] `scripts/build_release.ps1 -Version 0.2.16` creates the ZIP.
...
- v0.2.16 is not a single executable.
- v0.2.16 still uses a terminal-backed PowerShell launcher.
