...
Context Capsule v0.3.0 is distributed as a GitHub Release ZIP.
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File scripts\build_release.ps1 -Version 0.3.0 -DryRun
```
...
```text
Download context-capsule-v0.3.0.zip
Extract
...
If setup fails, v0.3.0 writes logs under `outputs\logs` and prints Korean guidance instead of leaving only a raw PowerShell exception.
...
- [ ] `scripts/build_release.ps1 -Version 0.3.0` creates the ZIP.
...
- v0.3.0 is not a single executable.
- v0.3.0 still uses a terminal-backed PowerShell launcher.
