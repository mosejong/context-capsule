...
Context Capsule v0.4.0 is distributed as a GitHub Release ZIP.
...
```text
dist/context-capsule-v0.4.0.zip
```

Dry-run:

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File scripts\build_release.ps1 -Version 0.4.0 -DryRun
```
...
```text
Download context-capsule-v0.4.0.zip
Extract
...
If setup fails, v0.4.0 writes logs under `outputs\logs` and prints Korean guidance instead of leaving only a raw PowerShell exception.
...
- [ ] `scripts/build_release.ps1 -Version 0.4.0` creates the ZIP.
...
- v0.4.0 is not a single executable.
- v0.4.0 still uses a terminal-backed PowerShell launcher.
