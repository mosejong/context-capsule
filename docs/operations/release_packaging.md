...
Context Capsule v0.3.1 is distributed as a GitHub Release ZIP.
...
```text
Download context-capsule-v0.3.1.zip
Extract
...
If setup fails, v0.3.1 writes logs under `outputs\logs` and prints Korean guidance instead of leaving only a raw PowerShell exception.
...
- [ ] `scripts/build_release.ps1 -Version 0.3.1` creates the ZIP.
...
- v0.3.1 is not a single executable.
- v0.3.1 still uses a terminal-backed PowerShell launcher.
