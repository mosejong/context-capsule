...
Use this file when publishing `Context Capsule v0.5.0` on GitHub Releases.
...
Tag: v0.5.0
Title: Context Capsule v0.5.0
Asset: dist/context-capsule-v0.5.0.zip
Release body: docs/releases/v0.5.0.md
```

## Before Upload

- [ ] Confirm `git status -sb` is clean.
- [ ] Confirm tag `v0.5.0` exists on GitHub.
- [ ] Confirm `dist/context-capsule-v0.5.0.zip` exists locally.
- [ ] Confirm ZIP was built with `scripts/build_release.ps1 -Version 0.5.0`.
- [ ] Confirm ZIP excludes `.venv`, `outputs`, `dist`, caches, and credentials.
- [ ] Copy the body from `docs/releases/v0.5.0.md`.
...
3. Select tag `v0.5.0`.
4. Set release title to `Context Capsule v0.5.0`.
5. Paste `docs/releases/v0.5.0.md` into the release description.
6. Upload `dist/context-capsule-v0.5.0.zip`.
7. Check that the asset name is exactly `context-capsule-v0.5.0.zip`.
8. Publish release.
...
- [ ] Confirm `README.md`, `START_HERE_KO.md`, `docs/reference/token_evidence.md`, `docs/reference/workflow_graph.md`, `docs/reference/work_handoff_ownership.md`, `docs/reference/nvidia_nim_provider.md`, `docs/releases/v0.5.0.md`, and `docs/local_app.md` exist.
...
v0.5.0 ships as a Windows ZIP with the v0.4 retrieval/evaluation features plus Task Contract generation, read-only contract verification, full external-fixture Context Recall coverage, and the run-scoped scale/app-web audit.
```
