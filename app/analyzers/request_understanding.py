...
ALIAS_RULES = [
    ...
    AliasRule(
        patterns=("심플 리트리버", "simple retriever", "simple_retriever", "검색", "리트리버"),
    ...
    AliasRule(
        patterns=("로컬 실행", "대시보드 실행", "실행 안돼", "실행 안됨", "run_dashboard", "launcher"),
        normalized="local launcher dashboard run",
        file_hints=("run_context_capsule.bat", "scripts/run_dashboard.ps1", "docs/local_app.md", "app/main.py"),
        target_hints=("local launcher",),
        intent="launcher_bug_investigation",
    ),
    AliasRule(
        patterns=("토큰 계산", "토큰 뻥튀기", "토큰 절감", "토큰 사용량", "token analyzer", "token budget"),
